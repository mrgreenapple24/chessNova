 ...
 * @brief Represents a single 16-byte record in a standard Polyglot opening book
 * file.
 */
typedef struct {
...
/**
 * @brief Generates a Polyglot-compatible Zobrist hash for the current board
 * position.
 *
...
/**
 * @brief Checks the opening book dynamically for a move matching the current
 * board state.
 *
 * @param board Pointer to the Board structure.
...
/**
 * @brief Initializes and loads the opening book from book_file_path into
 * memory.
 */
