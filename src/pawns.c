...
/**
 * @brief Calculates the destination squares for white pawns moving one square
 * forward.
...
/**
 * @brief Calculates the destination squares for white pawns moving two squares
 * forward.
 * @param wpawns Bitboard of white pawns.
 * @param empty Bitboard of all empty squares.
...
/**
 * @brief Calculates the destination squares for black pawns moving one square
 * forward.
...
/**
 * @brief Calculates the destination squares for black pawns moving two squares
 * forward.
 * @param bpawns Bitboard of black pawns.
 * @param empty Bitboard of all empty squares.
 * @return Bitboard of double push target squares.
...
/**
 * @brief Identifies white pawns that are capable of pushing forward two
 * squares.
 * @param wpawns Bitboard of white pawns.
 * @param empty Bitboard of empty squares.
...
/**
 * @brief Identifies black pawns that are capable of pushing forward two
 * squares.
 * @param bpawns Bitboard of black pawns.
 * @param empty Bitboard of empty squares.
 * @return Bitboard of pawns able to double push.
...
U64 wAnyLever(U64 wpawns, U64 bpawns) {
    return wEastLever(wpawns, bpawns) | wWestLever(wpawns, bpawns);
}
...
U64 bAnyLever(U64 bpawns, U64 wpawns) {
    return bEastLever(bpawns, wpawns) | bWestLever(bpawns, wpawns);
}
...
U64 wInnerLever(U64 wpawns, U64 bpawns) {
    ...
    return (wEastLever(wpawns, bpawns) & abcFiles) | (wWestLever(wpawns, bpawns) & fghFiles);
}
...
U64 wOuterLever(U64 wpawns, U64 bpawns) {
    ...
    return (wEastLever(wpawns, bpawns) & efgFiles) | (wWestLever(wpawns, bpawns) & bcdFiles);
}
...
U64 wCenterLever(U64 wpawns, U64 bpawns) {
    ...
    return (wEastLever(wpawns, bpawns) & dFile) | (wWestLever(wpawns, bpawns) & eFile);
}
...
U64 bInnerLever(U64 bpawns, U64 wpawns) {
    const U64 abcFiles = (0x0707070707070707);
    const U64 fghFiles = (0xE0E0E0E0E0E0E0E0);
    return (bEastLever(bpawns, wpawns) & abcFiles) | (bWestLever(bpawns, wpawns) & fghFiles);
}
...
U64 bOuterLever(U64 bpawns, U64 wpawns) {
    const U64 bcdFiles = (0x0E0E0E0E0E0E0E0E);
    const U64 efgFiles = (0x7070707070707070);
    return (bEastLever(bpawns, wpawns) & efgFiles) | (bWestLever(bpawns, wpawns) & bcdFiles);
}
...
U64 bCenterLever(U64 bpawns, U64 wpawns) {
    const U64 dFile = (0x0808080808080808);
    const U64 eFile = (0x1010101010101010);
    return (bEastLever(bpawns, wpawns) & dFile) | (bWestLever(bpawns, wpawns) & eFile);
}
...
/** @brief Returns pawns that have at least one neighbor on the file to the
 * East. */
...
/**
 * @brief Identifies pawn duos (two pawns of the same color side-by-side on the
 * same rank).
...
/** @brief Fills the entire file if it has atleast one white and atleast one
 * black pawn.
 * @param wpawns The white pawn bitboard.
 * @param bpawns The black pawn bitboard.
 * @return The bitboard after filling the entire file.
...
/** @brief Fills the portion of the file starting from and not including the
 * backward most white pawn (i.e the least advanced white pawn) to rank 8
...
/** @brief Fills the portion of the file starting from and not including the
 * forward most black most pawn (i.e the least advanced black pawn) to rank 1
...
/** @brief Fills the portion of the file starting from and not including the
 * backward most white pawn (i.e the least advanced white pawn) to rank 1
 * @param wpawns The white pawns bitboard
...
/** @brief Fills the portion of the file starting from and not including the
 * forward most black pawn (i.e the least advanced black pawn) to rank 8
 * @param bpawns The black pawns bitboard
...
U64 wEastAttackFrontSpans(U64 wpawns) {
    return wPawnEastAttacks(wFrontspans(wpawns));
}
...
U64 wWestAttackFrontSpans(U64 wpawns) {
    return wPawnWestAttacks(wFrontspans(wpawns));
}
...
U64 bEastAttackFrontSpans(U64 bpawns) {
    return bPawnEastAttacks(bFrontspans(bpawns));
}
...
U64 bWestAttackFrontSpans(U64 bpawns) {
    return bPawnWestAttacks(bFrontspans(bpawns));
}
...
U64 wEastAttackRearSpans(U64 wpawns) {
    return wPawnEastAttacks(southFill(wpawns));
}
...
U64 wWestAttackRearSpans(U64 wpawns) {
    return wPawnWestAttacks(southFill(wpawns));
}
...
U64 bEastAttackRearSpans(U64 bpawns) {
    return bPawnEastAttacks(southFill(bpawns));
}
...
U64 bWestAttackRearSpans(U64 bpawns) {
    return bPawnWestAttacks(southFill(bpawns));
}
...
U64 eastAttackFileFill(U64 pawns) {
    return shiftEast(fileFill(pawns));
}
...
U64 westAttackFileFill(U64 pawns) {
    return shiftWest(fileFill(pawns));
}
...
uint8_t fileSet(U64 pawns) {
    return (uint8_t)southFill(pawns);
}

/**
 * @brief returns the east files of pawns that are isolated (no pawns on either
 * side).
 */
...
/**
 * @brief returns the west files of pawns that are isolated (no pawns on either
 * side).
 */
...
/**
 * @brief returns the files of pawns that are isolated (no pawns on either
 * side).
 */
...
/**
 * @brief returns pawns with atleast one pawn in front and behind on the same
 * file.
 */
...
U64 isolanis(U64 pawns) {
    return noNeighbourOnEastFile(pawns) & noNeighbourOnWestFile(pawns);
}
