 ...
 *
 * This file handles parsing and querying Polyglot-compatible chess opening book
 * files (.bin). It reads the book into memory, calculates keys based on the
 * board, and returns randomly selected book moves.
 */
...
/**
 * @brief Piece code mapping from engine representation to Polyglot standard
 * format.
 *
 * Indexed by the engine's piece values: wp, wn, wb, wr, wq, wk, bp, bn, bb, br,
 * bq, bk, EMPTY.
 */
const int PolyKindOfPiece[13] = {1, 3, 5, 7, 9, 11, 0, 2, 4, 6, 8, 10, -1};
 ...
 * If successful, determines the size, allocates memory, and reads all entries
 * into memory.
 */
...
/**
 * @brief Checks if there is a capturing pawn available for the active side to
 ...
 *
 * Hashing includes pieces placement, castling permissions, en passant target
 * squares (only when capturable), and the active player's side to move.
 *
 * @param board Pointer to the Board structure.
...
/**
 * @brief Swaps the byte-order of a 16-bit unsigned integer (big-endian to
 * little-endian).
 *
...
/**
 * @brief Swaps the byte-order of a 64-bit unsigned integer (big-endian to
 * little-endian).
 *
...
/**
 * @brief Converts a 16-bit Polyglot move representation to our engine's packed
...
uint32_t ConvertPolyMoveToInternalMove(unsigned short polyMove, Board* board) {
    ...
    if (pp == 0) {
        ...
        sprintf(moveString, "%c%c%c%c%c", 'a' + ff, '1' + fr, 'a' + tf, '1' + tr, promChar);
    }
 ...
 *
 * @param board Pointer to the Board structure.
 * @return uint32_t A packed 32-bit move found in the book, or 0 if no moves are
 * found.
 */
uint32_t GetBookMove(Board* board) {
    if (!use_book || entries == NULL || NumEntries == 0)
        return 0;
