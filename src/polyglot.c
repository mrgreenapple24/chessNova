...
/*
 * Standard Polyglot random numbers (781 total)
...
uint64_t polyglot_hash(Board *board) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (piece != EMPTY) {
            ...
            hash ^= polyglot_random[64 * idx + sq];
        }
    }

    if (board->castle & WKCA) hash ^= polyglot_random[768];
    if (board->castle & WQCA) hash ^= polyglot_random[769];
    if (board->castle & BKCA) hash ^= polyglot_random[770];
    if (board->castle & BQCA) hash ^= polyglot_random[771];
    ...
    if (board->side == white) {
        // White to move, look for Black pawns that can capture
        if (file > 0 && board->pieces[board->enpassant - 9] == bp) can_capture = true;
        if (file < 7 && board->pieces[board->enpassant - 7] == bp) can_capture = true;
    } else {
        // Black to move, look for White pawns that can capture
        if (file > 0 && board->pieces[board->enpassant + 7] == wp) can_capture = true;
        if (file < 7 && board->pieces[board->enpassant + 9] == wp) can_capture = true;
    }
