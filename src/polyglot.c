/**
 * @file polyglot.c
 * @brief Polyglot Zobrist hash generator and dynamic book move lookup
 * implementation.
 *
 * Provides functions to generate Zobrist hashes conforming to the Polyglot
 * opening book specification, as well as functions to dynamically search and
 * read entries from a Polyglot book binary on disk.
 */
...
/**
 * @brief Maps an engine piece value to its corresponding Polyglot piece format
 * code.
 *
 ...
 *
 * Includes active pieces, castling permissions, en passant squares (if
 * capturable), and side to move (White).
 *
...
/**
 * @brief Checks the opening book for a move by dynamically scanning the book on
 * disk using binary search.
 *
 * @param board Pointer to the Board structure.
...
uint32_t get_polyglot_move(Board* board) {
    ...
    while (low <= high) {
        ...
        if (fread(&entry, sizeof(PolyglotEntry), 1, fp) != 1)
            break;
